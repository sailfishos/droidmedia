...
void _DroidMediaBufferQueue::frameAvailable()
{
    ...
    if (item.mGraphicBuffer != NULL) {
        ...
        if (slot.droidBuffer.get()) {
            ...
            // Recreate the user data if it was cleared for some reason like the
            // buffer pool resetting itself without the queue being recreated.
            // In reality buffer pools may always outlive queue so may never
            // trigger.
