...
public:
    ...
    virtual status_t setDeviceConnectionState(audio_devices_t device,
        audio_policy_dev_state_t state, const char *device_address, const char *device_name)
    {
        return NO_ERROR;
    }
    ...
    virtual audio_io_handle_t getOutput(audio_stream_type_t stream, uint32_t samplingRate = 0,
        audio_format_t format = AUDIO_FORMAT_DEFAULT, audio_channel_mask_t channelMask = 0,
        audio_output_flags_t flags = AUDIO_OUTPUT_FLAG_NONE,
        const audio_offload_info_t *offloadInfo = NULL)
    {
        return AUDIO_IO_HANDLE_NONE;
    }
    virtual status_t getOutputForAttr(const audio_attributes_t *attr, audio_io_handle_t *output,
        audio_session_t session, audio_stream_type_t *stream, uid_t uid, uint32_t samplingRate = 0,
        audio_format_t format = AUDIO_FORMAT_DEFAULT, audio_channel_mask_t channelMask = 0,
        audio_output_flags_t flags = AUDIO_OUTPUT_FLAG_NONE,
    ...
    virtual status_t getInputForAttr(const audio_attributes_t *attr, audio_io_handle_t *input,
        audio_session_t session, pid_t pid, uid_t uid, uint32_t samplingRate, audio_format_t format,
        audio_channel_mask_t channelMask, audio_input_flags_t flags,
        audio_port_handle_t selectedDeviceId = AUDIO_PORT_HANDLE_NONE)
    {
        return NO_ERROR;
    }
    virtual status_t startInput(audio_io_handle_t input, audio_session_t session)
    {
        return NO_ERROR;
    }
    virtual status_t stopInput(audio_io_handle_t input, audio_session_t session)
    {
        return NO_ERROR;
    }
    ...
    virtual status_t queryDefaultPreProcessing(
        audio_session_t audioSession, effect_descriptor_t *descriptors, uint32_t *count)
    {
        return NO_ERROR;
    }
    ...
    virtual status_t startAudioSource(const struct audio_port_config *source,
        const audio_attributes_t *attributes, audio_io_handle_t *handle)
    {
        return NO_ERROR;
    }
    ...
    void onRecordingConfigurationUpdate(int event, audio_session_t session, audio_source_t source,
        const audio_config_base_t *clientConfig, const audio_config_base_t *deviceConfig,
        audio_patch_handle_t patchHandle)
    {
        return;
    }
    void doOnRecordingConfigurationUpdate(int event, audio_session_t session, audio_source_t source,
        const audio_config_base_t *clientConfig, const audio_config_base_t *deviceConfig,
        audio_patch_handle_t patchHandle)
    {
        return;
    ...
    virtual status_t StartOutputSamplerate(audio_io_handle_t output, audio_stream_type_t stream,
        audio_session_t session, int samplerate)
    {
        return NO_ERROR;
    }
    virtual status_t StopOutputSamplerate(audio_io_handle_t output, audio_stream_type_t stream,
        audio_session_t session, int samplerate)
    {
        return NO_ERROR;
    }
    virtual status_t SampleRateRequestFocus(
        audio_io_handle_t output, audio_stream_type_t stream, int *samplerate)
    {
        return NO_ERROR;
    }
    virtual status_t SampleRateUnrequestFocus(
        audio_io_handle_t output, audio_stream_type_t stream, int samplerate)
    {
        return NO_ERROR;
    }

    virtual status_t doStopOutputSamplerate(audio_io_handle_t output, audio_stream_type_t stream,
        audio_session_t session, int samplerate)
    {
        return NO_ERROR;
    }
    virtual status_t getCustomAudioVolume(void *pCustomVol) { return NO_ERROR; }
#endif
};
