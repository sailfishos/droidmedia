...
public:
    ...
    sp<ISurfaceComposerClient> createScopedConnection(const sp<IGraphicBufferProducer> &parent)
    {
    ...
    status_t enableVSyncInjections(bool enable) { return BAD_VALUE; }

    status_t injectVSync(nsecs_t when) { return BAD_VALUE; }
};
...
public:
    ...
    virtual int32_t configureChannel(int32_t handle, int32_t rateLevel) { return 0; }
...
public:
    ...
    Vector<Sensor> getDynamicSensorList(const String16 &opPackageName) { return Vector<Sensor>(); }
    ...
    int setOperationParameter(int32_t handle, int32_t type, const Vector<float> &floats,
