...
struct _DroidMediaCamera
{
    ...
    android::sp<DroidMediaBufferQueue> m_video_queue;
...
DroidMediaBufferQueue *droid_media_camera_get_video_buffer_queue (DroidMediaCamera *camera)
{
  return camera->m_video_queue.get();
}
...
#if ANDROID_MAJOR >= 8
    android::sp<DroidMediaBufferQueue>
      video_queue(new DroidMediaBufferQueue("DroidMediaCameraBufferVideoQueue"));
    if (!video_queue->connectListener()) {
        ALOGE("Failed to connect video buffer queue listener");
    } else {
      cam->m_video_queue = video_queue;
      cam->m_video_queue->attachToCameraVideo(cam->m_camera);
    }
#endif
...
#if ANDROID_MAJOR >= 8
    if (camera->m_video_queue.get()) {
      camera->m_video_queue->setCallbacks(0, 0);
    }
#endif
